...
def _table_rows(table_name: str) -> tuple[list[str], list[dict[str, Any]]]:
    ...
    if not columns:
        return [], []
    order_column = "user_id" if "user_id" in columns else columns[0]
    with SessionLocal() as db:
        rows = db.execute(text(f"SELECT * FROM {table_name} ORDER BY {order_column}")).mappings().all()
        return columns, [dict(row) for row in rows]


def _build_login_export_text(generated_at: datetime) -> tuple[str, int]:
    lines: list[str] = []
    total = 0
    lines.append("TR4 /listening - exportacao integral de logins")
    lines.append(f"Gerado em UTC: {generated_at.isoformat(sep=' ')}")
    lines.append(f"Corte legacy: {LEGACY_CUTOFF.isoformat(sep=' ')} UTC")
    lines.append("")
    for table_name in _LOGIN_TABLES:
        columns, rows = _table_rows(table_name)
        total += len(rows)
        lines.append(f"[{table_name}] linhas={len(rows)} colunas={', '.join(columns) if columns else 'tabela ausente'}")
        if not rows:
        ...
        for index, row in enumerate(rows, start=1):
            ...
            lines.append("")
    columns, rows = _table_rows("legacy_restricted_users")
    lines.append(f"[legacy_restricted_users] linhas={len(rows)} colunas={', '.join(columns) if columns else 'tabela ausente'}")
    for index, row in enumerate(rows, start=1):
        lines.append(f"  #{index}")
        for column in columns:
            lines.append(f"    {column}: {_stringify(row.get(column))}")
...
def _bundle_from_lines(*, lines: list[str], stamp: str, row_count: int, suffix: str = "") -> ExportBundle:
    text_body = "\n".join(lines).rstrip() + "\n"
...
def build_listening_export_parts(max_document_bytes: int = MAX_TELEGRAM_DOCUMENT_BYTES) -> list[ExportBundle]:
    now = utcnow_naive()
    text_body, row_count = _build_login_export_text(now)
    lines = text_body.splitlines()
    stamp = now.strftime("%Y%m%d-%H%M%S")
    single = _bundle_from_lines(lines=lines, stamp=stamp, row_count=row_count)
    ...
    for index, chunk in enumerate(content_chunks, start=1):
        prefixed = [
            ...
            f"Registros de login no banco: {row_count}",
            "",
            *chunk,
        ]
        bundle = _bundle_from_lines(lines=prefixed, stamp=stamp, row_count=row_count, suffix=f"part{index:03d}-of-{total:03d}")
        ...
        for sub_index, sub_chunk in enumerate(sub_chunks, start=1):
            ...
            sub_prefixed = [f"TR4 /listening - parte {index}/{total}.{sub_index}", f"Registros de login no banco: {row_count}", "", *sub_chunk]
            bundles.append(_bundle_from_lines(lines=sub_prefixed, stamp=stamp, row_count=row_count, suffix=suffix))
    return bundles

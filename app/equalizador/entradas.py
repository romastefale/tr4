...
def reset_join_request_owner(*, palco: dict[str, object], entrada_ref: str, ator_ref: str, alias_secret: str, db_engine: Engine = default_engine) -> dict[str, object]:
    """Owner-only local reset of a captured join-request row.

    This does not approve/decline anything on Telegram. It only reopens a
    captured request in TR4's local queue after the owner decides it should be
    reviewed again. Telegram remains the source of truth for actual entry.
    """
    ensure_phase43_tables(db_engine)
    palco_id = int(palco["telegram_chat_id"])
    row = resolve_join_request(palco_id=palco_id, entrada_ref=entrada_ref, db_engine=db_engine)
    with db_engine.begin() as conn:
        conn.execute(
            text("UPDATE eq_join_requests SET estado='pendente', updated_at=:updated_at WHERE id=:id"),
            {"updated_at": _now_iso(), "id": int(row["id"])},
        )
    history = record_historico(
        ator_ref=ator_ref,
        palco_ref=str(palco["ui_ref"]),
        alvo_ref=str(row["ui_ref"]),
        ajuste="entradas.resetar",
        status="concluido",
        resumo_publico=f"Pedido de entrada reaberto: {row['nome_publico']}",
        payload_tecnico={"local_only": True},
        alias_secret=alias_secret,
        db_engine=db_engine,
    )
    return {"ok": True, "entrada": {"entrada_ref": row["ui_ref"], "nome": row["nome_publico"], "situacao": "pendente"}, "historico_ref": history["historico_ref"], "resumo": history["resumo"]}


async def criar_link_direto_owner(*, palco: dict[str, object], ator_ref: str, bot_token: str, alias_secret: str, nome: str = "Owner direct link", member_limit: int = 1, db_engine: Engine = default_engine, api_call: TelegramApiCallable = telegram_api_call) -> dict[str, object]:
    """Create a direct invite link without join-request approval. Owner-only caller.

    /tctl uses this for the TR3 parity item “link direto”. For normal group
    operation prefer approval links; direct links stay owner-only.
    """
    palco_id = int(palco["telegram_chat_id"])
    payload: dict[str, Any] = {
        "chat_id": palco_id,
        "name": _safe_text(nome, fallback="Owner direct link")[:32],
    }
    if member_limit > 0:
        payload["member_limit"] = max(1, min(int(member_limit), 99999))
    try:
        result = await api_call(bot_token, "createChatInviteLink", payload)
        invite_ref = register_invite_from_result(chat_id=palco_id, result=result if isinstance(result, dict) else {}, alias_secret=alias_secret, db_engine=db_engine)
        history = record_historico(
            ator_ref=ator_ref,
            palco_ref=str(palco["ui_ref"]),
            alvo_ref=invite_ref,
            ajuste="convites.link_direto",
            status="concluido",
            resumo_publico="Link direto owner criado",
            payload_tecnico={"method": "createChatInviteLink", "creates_join_request": False, "member_limit": payload.get("member_limit")},
            alias_secret=alias_secret,
            db_engine=db_engine,
        )
        return {"ok": True, "convite": str((result or {}).get("invite_link") or ""), "historico_ref": history["historico_ref"], "resumo": history["resumo"]}
    except Exception as exc:
        detail = entradas_error_public_detail(exc)
        record_historico(
            ator_ref=ator_ref,
            ...
            ajuste="convites.link_direto",
            status="falhou",
            resumo_publico=f"Link direto não criado · {detail}",
            payload_tecnico={"method": "createChatInviteLink", "creates_join_request": False, "motivo_publico": detail, **(telegram_error_payload(exc.info) if isinstance(exc, EntradasTelegramError) and getattr(exc, "info", None) else {})},
            alias_secret=alias_secret,
            db_engine=db_engine,
        )
        raise

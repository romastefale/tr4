...
(function(){
  ...
  async function prepareDownloadUrl(target,filename){if(new RegExp("^https?://","i").test(target))return target;const res=await api("/equalizador/api/public/download-result",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({target:target,filename:filename}),timeoutMs:30000});return absoluteUrl(res.download_url||"");}

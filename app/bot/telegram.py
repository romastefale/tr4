...
def _start_text(message: Message) -> str:
    if message.chat.type != "private":
        return (
            ...
            "<b>Conectar perfil musical:</b> <code>/lastfm seu_usuario</code>\n"
    ...
    if _is_owner_message(message):
        return (
            "♫ ♥ <b>tigraoRADIO</b>\n\n"
            "Sua central musical está ativa. Use esta conversa para conectar sua conta musical, "
            "acompanhar sua música atual, gerar cards, extratos e rankings por DM.\n\n"
            "<b>Conectar perfil musical:</b> <code>/lastfm seu_usuario</code>\n"
            "<b>Conectar conta musical:</b> <code>/login</code>\n"
    ...
    return (
        ...
        "Conecte seu perfil musical para acompanhar o que você está ouvindo, gerar cards, "
...
def _help_text(message: Message) -> str:
    if message.chat.type != "private":
        return (
            ...
            "<code>/tcanvas</code> — envia o canvas da música atual.\n"
            ...
            "<code>/weekfm</code> — mostra seu extrato semanal.\n"
            "<code>/monthfm</code> — mostra seu extrato mensal.\n"
            "<code>/songcharts</code> — mostra o ranking musical do grupo.\n"
            "<code>/lastfm</code> — conecta ou atualiza seu perfil musical.\n"
            "<code>/lastfmoff</code> — remove seu perfil musical.\n"
    ...
    if _is_owner_message(message):
        return (
            ...
            "<code>/lastfm</code> — conecta ou atualiza seu perfil musical.\n"
            "<code>/lastfmoff</code> — remove seu perfil musical.\n"
            "<code>/login</code> — conecta sua conta musical.\n"
            "<code>/logout</code> — remove sua conexão musical.\n"
            "<code>/playing</code> — mostra sua música atual.\n"
            "<code>/albnow</code> — destaca o álbum da música atual.\n"
            "<code>/tcanvas</code> — envia o canvas da música atual.\n"
            "<code>/tstory</code> — monta story da música atual.\n"
...
def _register_handlers(dp: Dispatcher) -> None:
    ...
    async def start(message: Message) -> None:
        ...
        #   - connect           → mostra fluxo de conexão (Last fm + conta musical)
        ...
        if payload.startswith("lastfm_") and message.from_user:
            ...
            try:
                ...
                await message.answer(
                    f"{mention}, usuário inválido para <code>/lastfm</code>: "
                    f"<code>{html.escape(raw_username)}</code>",
                    parse_mode="HTML",
                )
                return
            if previous and previous.lower() == username.lower():
                head = f"{mention}, perfil musical reconfirmado."
            elif previous:
                head = f"{mention}, perfil musical atualizado."
            else:
                head = f"{mention}, perfil musical conectado."
        ...
        if payload == "connect":
            await message.answer(
                ...
                "<code>/lastfm seu_usuario</code> (sem @)\n"
                "<code>/login</code>\n\n"
    ...
    async def login(message: Message) -> None:
        if message.chat.type != "private":
            await message.answer(
                ...
                "1) <code>/login</code>\n"
                "2) <code>/lastfm seu_usuario</code> (sem @)\n\n"
        ...
        await message.answer(
            "🎧 <b>Conectando sua conta musical</b>\n\n"
            f'1) <a href="{html.escape(auth_url, quote=True)}">Abrir autorização</a>\n\n'
            "2) Depois mande aqui:\n"
            "<code>/lastfm seu_usuario</code>  (sem @)\n\n"
            "Depois rode <code>/playing</code> para testar.",
            parse_mode="HTML",
            disable_web_page_preview=True,
        )
    ...
    async def logout(message: Message) -> None:
        ...
        await message.answer("Conexão musical removida.")
    ...
    async def lastfm(message: Message) -> None:
        ...
        if len(parts) < 2:
            ...
            if current:
                await message.answer(
                    f"{mention}, seu perfil musical já está conectado.\n"
                    "Para atualizar: <code>/lastfm outro_usuario</code> (sem @).\n"
                    "Para remover: /lastfmoff",
                    parse_mode="HTML",
                )
            else:
                await message.answer(
                    f"{mention}, conecte seu perfil musical.\n\n"
                    "🎧 <b>Como usar:</b>\n"
                    "<code>/lastfm seu_usuario</code> (sem @).\n\n"
                    "Depois rode o comando de novo.",
                    parse_mode="HTML",
        ...
        try:
            ...
            await message.answer(f"{mention}, usuário inválido para <code>/lastfm</code>.", parse_mode="HTML")
            return
        if previous and previous.lower() == username.lower():
            head = f"{mention}, perfil musical reconfirmado."
        elif previous:
        ...
        if not LASTFM_API_KEY:
            await message.answer(
                f"{head}\n\n"
                "A conexão técnica ainda precisa ser configurada no servidor. "
    ...
    async def lastfmoff(message: Message) -> None:
        ...
        await _answer_with_effect(
            message,
            f"{mention}, perfil musical removido." if removed else f"{mention}, nenhum perfil musical estava conectado.",
            _EFFECT_THUMBS_UP,
            parse_mode="HTML",
        )
    ...
    async def plays_callback(query: CallbackQuery) -> None:
        ...
        if plays_source == "lastfm":
            await query.answer("O número azul é o total do seu histórico musical.\nPelo bot: " + str(count) + " vez" + ("" if count == 1 else "es") + ".", show_alert=True)
        else:

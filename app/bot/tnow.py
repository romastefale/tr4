...
- O nome visual do card usa perfil Telegram persistido; username musical é
  somente dado técnico de busca/cache e não é fallback visual.
"""
...
# Limite duro para evitar requests excessivos. A seleção final usa até
# 25 usuários válidos; o layout apenas acomoda visualmente a quantidade real.
MAX_USERS = 60
MAX_TILES = 25
...
# Layout adaptativo: a grade não filtra usuários válidos. Ela apenas escolhe
# uma matriz com capacidade suficiente e pouco vazio visual para 1..25 tiles.
...
def _choose_grid_layout(n: int) -> tuple[int, int, int]:
    """Retorna (linhas, colunas, capacidade) sem cortar usuário válido."""
    if n <= 0:
        return 0, 0, 0
    needed = min(max(1, int(n)), MAX_TILES)
    for rows, columns in _TNOW_GRID_LAYOUTS:
        capacity = rows * columns
        if capacity >= needed:
            return rows, columns, capacity
    return 5, 5, MAX_TILES


def _grid_slots(n: int) -> int:
    """Capacidade da menor grade adaptativa que comporta n tiles."""
...
def _lastfm_display_name(user_id: int) -> str | None:
    """Technical lookup for the connected musical username used by cache/logging."""
    try:
        with SessionLocal() as db:
            username = db.execute(
...
async def _display_name(bot: Any, user_id: int, lastfm_username: str | None = None, *, surface: str = "tnow") -> str:
    """Nome visual do mosaico/tnow sem expor username musical.

    `lastfm_username` permanece no parâmetro por compatibilidade com o fluxo de
    cache/log, mas não é usado como fallback visual. A ordem real fica: /tpv,
    perfil Telegram salvo, get_chat reparador, User.
    """
    return await telegram_user_profile_service.resolve_music_display_name(
        bot,
        user_id,
        surface=surface,
    )
...
async def _gather_entries(bot: Any, chat: Chat | None = None, *, use_snapshot: bool = True) -> list[TnowEntry]:
    ...
    selected_activities = eligible[:MAX_TILES]
    rows, columns, capacity = _choose_grid_layout(len(selected_activities))
    empty_slots = max(0, capacity - len(selected_activities))

    # Aquece o cache de canal apenas para os selecionados. O corte só ocorre
    # acima de 25 usuários válidos; a grade não remove usuário dentro do limite.
    ...
    for activity in eligible:
        logger.info(
            "TNOW_ENTRY_DECISION | user_id=%s | lastfm_username=%s | source=%s | status=%s | age_seconds=%s | selected=%s | grid=%sx%s | capacity=%s | track=%s | artist=%s",
            activity.user_id,
    ...
    logger.info(
        "TNOW_GRID_SELECTED | scope=%s | eligible=%s | grid=%sx%s | capacity=%s | rendered=%s | empty_slots=%s | live=%s | recent_15=%s | recent_30=%s | recent_45=%s | recent_120=%s | chat_id=%s",
        scope,
        len(eligible),
        rows,
        columns,

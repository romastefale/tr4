...
async def execute_nowp_publish(
    ...
    *,
    ...
    if not is_user_connected(requester_id):
        raise MusicCommandError("not_connected", "Use /lastfm seu_usuario ou /login antes de usar este comando.", status_code=403)
...
async def _run_universal_songcharts_task(
    ...
    *,
    ...
    try:
        status = await bot.send_message(
            requester_id,
            f"Gerando ranking universal do {label} com todos os perfis musicais importados...",
            parse_mode="HTML",
        )
    except Exception:
...
async def current_track_preview(user_id: int) -> dict[str, Any]:
    if not is_user_connected(user_id):
        return {
            "available": False,
            "code": "not_connected",
            "message": "Use /lastfm seu_usuario ou /login para ver sua música atual.",
        }

        ...
        try:
            if message.bot:
                await message.bot.send_message(
                    ...
                    "Use <code>/tmn user_id username_lastfm</code> na DM do bot.",
                    parse_mode="HTML",
                )
        except Exception:
...
async def tmn_manual_register(message: Message) -> None:
    ...
    try:
        ...
        await message.answer(
            "Uso correto:\n"
            "<code>/tmn user_id username_lastfm</code>\n\n"
            "Exemplo:\n"
            "<code>/tmn 8505890439 romastefale</code>\n"
            "Também aceita <code>@username</code> ou URL do Last.fm.",
            parse_mode="HTML",
        )
        return
    ...
    try:
        ...
        await message.answer("Falha ao cadastrar manualmente. Verifique o user_id e o username Last.fm.")
        return
    ...
    await message.answer(
        ...
        f"Last.fm: <code>{html.escape(clean_username)}</code>\n"
        f"Limpeza: lastfm_profiles={lastfm_deleted}, spotify_tokens={spotify_deleted}",
        parse_mode="HTML",
    )

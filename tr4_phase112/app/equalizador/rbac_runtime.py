...
def governance_persistence_public(*, alias_secret: str, db_engine: Engine = default_engine) -> dict[str, object]:
    """Owner-only persistence summary for governantes and runtime grants.

    The payload is intentionally translated/sanitized: it exposes counts and public
    status only, never raw Telegram IDs, database paths, tokens, or grant internals.
    """
    ensure_rbac_runtime_tables(db_engine)
    with db_engine.begin() as conn:
        operadores_ativos = int(conn.execute(text("SELECT COUNT(*) FROM eq_operadores WHERE habilitado=1")).scalar() or 0)
        operadores_desativados = int(conn.execute(text("SELECT COUNT(*) FROM eq_operadores WHERE habilitado=0")).scalar() or 0)
        concessoes_ativas = int(conn.execute(text("SELECT COUNT(*) FROM eq_runtime_grants WHERE habilitado=1")).scalar() or 0)
        concessoes_revogadas = int(conn.execute(text("SELECT COUNT(*) FROM eq_runtime_grants WHERE habilitado=0")).scalar() or 0)
        auditoria = int(conn.execute(text("SELECT COUNT(*) FROM eq_governance_audit")).scalar() or 0)
        ultimo_evento = conn.execute(text("SELECT action, public_detail, created_at FROM eq_governance_audit ORDER BY created_at DESC, id DESC LIMIT 1")).mappings().first()
    status = "persistente" if (operadores_ativos or concessoes_ativas or auditoria) else "sem movimentação runtime"
    return {
        "ok": True,
        "status": status,
        "resumo": {
            "governantes_ativos": operadores_ativos,
            "governantes_desativados": operadores_desativados,
            "concessoes_ativas": concessoes_ativas,
            "concessoes_revogadas": concessoes_revogadas,
            "eventos_auditoria": auditoria,
        },
        "ultimo_evento": {
            "acao": str(ultimo_evento["action"] or "") if ultimo_evento else "",
            "detalhe": str(ultimo_evento["public_detail"] or "") if ultimo_evento else "",
            "quando": str(ultimo_evento["created_at"] or "") if ultimo_evento else "",
        },
        "observacao": "Governança runtime gravada no banco persistente; variáveis do Railway continuam como base estável.",
    }

...
def test_tmn_command_criado_owner_only():
    ...
    assert 'CommandDef("tmn", "Cadastrar perfil musical manualmente")' in SETUP
...
def test_tmn_uso_documentado_no_handler():
    assert '/tmn user_id perfil_musical' in TMN
    assert '/tmn 8505890439 usuario_exemplo' in TMN

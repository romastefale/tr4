...
def test_tnow_grade_completa_usa_sequencia_owner_sem_usuario_falso():
    assert "_TNOW_GRID_LAYOUTS" in TNOW
    for layout in ("(5, 5)", "(5, 4)", "(4, 4)", "(3, 4)", "(3, 3)", "(2, 4)", "(1, 1)"):
        assert layout in TNOW
    assert "_choose_grid_layout" in TNOW
    assert "selected_activities = eligible[:slots]" in TNOW
